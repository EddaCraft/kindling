...
pub async fn run(profile: Profile) -> Result<BenchmarkReport, BoxError> {
    let groups = vec![
        cold_start_group(&profile).await?,
        direct_service_group(&profile)?,
        daemon_group(&profile).await?,
        spool_group(&profile).await?,
    ];
    Ok(BenchmarkReport {
        schema_version: 1,
...
async fn cold_start_group(profile: &Profile) -> Result<WorkloadGroup, BoxError> {
    let sampler = ResourceSampler::start(Duration::from_millis(2));
...
fn direct_service_group(profile: &Profile) -> Result<WorkloadGroup, BoxError> {
    let temp = tempfile::tempdir()?;
...
async fn daemon_group(profile: &Profile) -> Result<WorkloadGroup, BoxError> {
    let temp = tempfile::tempdir()?;
    let runtime = Runtime::start(RuntimeConfig::with_home(
        temp.path(),
        PROJECT_ROOT,
...
async fn spool_group(profile: &Profile) -> Result<WorkloadGroup, BoxError> {
    let temp = tempfile::tempdir()?;
