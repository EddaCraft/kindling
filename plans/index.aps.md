...
- **CodeQL `rust/cleartext-logging` #11–#14** — taint through `mask_secrets` / `mask_secrets_with_evidence` into `assert!` / CLI `println!`. All false positives after redaction (or used-in-tests). Disposition in `SECURITY.md`; CodeQL model pack at `.github/codeql/extensions/kindling-rust/`.
